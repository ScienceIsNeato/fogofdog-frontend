...
# Before committing (slop-mop based validation)
./scripts/validate.sh               # Full quality gate check (JS + TypeScript + duplication)
...
```bash
# Quality gate using slop-mop (recommended before commits)
./scripts/validate.sh                              # Full quality check (JS + TypeScript + duplication)
./scripts/validate.sh quick                        # Quick lint-only check
./scripts/validate.sh js                           # JavaScript validation only
./scripts/validate.sh full                         # All quality gates

# Direct slop-mop commands (requires venv activation)
source .venv/bin/activate && sm validate javascript  # JavaScript profile
source .venv/bin/activate && sm validate quick       # Quick lint check
...
npm run duplication:check                          # Code duplication analysis
```
