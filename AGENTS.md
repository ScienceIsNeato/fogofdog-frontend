...
>
> **Last Updated:** 2026-02-11 17:38:38 UTC  
...
1. **Use slop-mop for coverage checks**: Run `sm validate -g <language>:coverage` (never ad-hoc coverage commands)
...
```bash
sm validate -g pr:comments  # Fails if unaddressed PR comments exist
```

### AI Implementation Protocol

When `sm validate -g pr:comments` fails due to unaddressed PR comments:
...
7. **Iterate**: Re-run `sm validate -g pr:comments`, repeat until no unaddressed comments remain
...
**Surface level (doesn't work)**: Memorize "don't bypass quality gates"
...
```bash
# Run the pr:comments gate to get categorized, actionable report
sm validate -g pr:comments
