...
function validateGPSData(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (typeof point.latitude !== 'number' || 
      ...
      throw new Error(`Invalid GPS point at index ${i}: requires latitude, longitude, and timestamp as numbers`);
    }
    
    if (point.latitude < -90 || point.latitude > 90) {
      throw new Error(`Invalid latitude at index ${i}: ${point.latitude} (must be between -90 and 90)`);
    }
    
    if (point.longitude < -180 || point.longitude > 180) {
      throw new Error(`Invalid longitude at index ${i}: ${point.longitude} (must be between -180 and 180)`);
    }
...
async function injectGPSData(data, options) {
  try {
    ...
    console.log(`📊 Time span: ${new Date(Math.min(...processedData.map(p => p.timestamp))).toISOString()} to ${new Date(Math.max(...processedData.map(p => p.timestamp))).toISOString()}`);
    ...
    console.log(`AsyncStorage.setItem('${GPS_INJECTION_KEY}', '${jsonData.replace(/'/g, "\\'")}');`);
