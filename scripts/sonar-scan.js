...
function displayIssues(issues) {
  ...
  severityOrder.forEach((severity) => {
    if (groupedIssues[severity]) {
      ...
      groupedIssues[severity].slice(0, 10).forEach((issue) => {
        // Show first 10 per severity
  ...
  console.log(
    '🎯 View all details at: https://sonarcloud.io/project/issues?id=ScienceIsNeato_fogofdog-frontend&resolved=false'
  );
}
...
scanner(
  {
    ...
    options: {
      ...
      'sonar.exclusions':
        '**/node_modules/**,**/*.test.ts,**/*.test.tsx,**/__tests__/**,**/__mocks__/**,coverage/**,artifacts/**,e2e/**,ios/**,android/**,.expo/**,dist/**',
  ...
  async (result) => {
    ...
    if (!result) {
      ...
      console.log(
        '🎯 View results at: https://sonarcloud.io/project/overview?id=ScienceIsNeato_fogofdog-frontend'
      );
      process.exit(0);
    }
    ...
    if (result.status === 'SUCCESS') {
      ...
      console.log(
        '🎯 Check the results at: https://sonarcloud.io/project/overview?id=ScienceIsNeato_fogofdog-frontend'
      );
      process.exit(0);
    } else {
      if (TREAT_AS_WARNING) {
        ...
        console.log(
          '🔗 Bug report: https://community.sonarsource.com/t/currently-sonarcloud-does-not-show-coverage-analysis-on-each-pull-request/114064'
        );
